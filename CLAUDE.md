...
- **Overwrite protection**: the ffmpeg commands still use `-y`, but the app gates the invocation. `colliding_input()` refuses when the output path resolves to one of the input clips (ffmpeg "cannot edit in-place" — always fatal, no `--force`). An existing output aborts the CLI unless `-f/--force`; the GUI asks via `messagebox.askyesno` (existence check in `_start` on the main thread, collision check in the worker after `discover_clips`). Don't remove these and rely on `-y` alone — it silently clobbers and crashes on the in-place case.
